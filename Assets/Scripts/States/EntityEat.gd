...
func Enter():
	get_parent().get_parent().get_node("ShapeKeys").play("Chomping")
	entity.rcp("play_sound","Eat","res://Assets/SFX/dramaticStab.mp3",rng.randf_range(.9,1.1))
	randomize_wander()
...
func Exit():
	rpc("drop",entity.carrying.stats.id)

func Physics_Update(delta: float):
	if entity.carrying:
		entity.carrying.global_position = entity.carrying.global_position.lerp(jawSlot.global_position,delta*15)
...
func bite(dmg):
	entity.carrying.stats.damage(dmg,entity.global_position)
